...
def login():
    ...
    def register_sql():
        global username_confirm
        # SQL
        cursor = mydb.cursor()
        # Commande sql pour insérer les données
        sql ="""
                INSERT INTO users(users.username, users.`password`, users.`level`)
                VALUES (%s, %s, 2);
            """
        cursor.execute(sql, (username_entry.get(), hash_password(password_entry.get()), ))

        sql = "SELECT users.username FROM users WHERE username = %s"
        cursor.execute(sql, (username_entry.get(), ))
        username_confirm = cursor.fetchall()

        if username_confirm == username_entry.get():
            print("Ce pseudo existe déjà, Loge toi fdp !")
    ...
    fenetre_login.title("Register")
    fenetre_login.geometry("300x200")
    fenetre_login.minsize(300, 300)
    fenetre_login.maxsize(300, 300)

    # Création du label et de l'entrée pour le pseudo.
    username_label = Label(fenetre_login, text="Pseudo :")
    username_label.pack()
    username_entry = Entry(fenetre_login, width=30)
    username_entry.pack()

    # Création du label et de l'entrée pour le mot de passe.
    password_label = Label(fenetre_login, text="Mot de passe :")
    password_label.pack()
    password_entry = Entry(fenetre_login, width=30)
    password_entry.pack()

    # Création du bouton de validation
    validation_button = Button(fenetre_login, text="Valider", command=register_sql)
    validation_button.pack(side=BOTTOM, padx=5, pady=5)
